...
namespace X3Platform.HttpServer
{
    public class HttpServer : MarshalByRefObject
    {
        ...
        string _staticFileFilters;
        string _directoryBrowse;

        bool _shutdownInProgress;
        Socket _socket;
        Host _host;

        public HttpServer(int port, string virtualPath, string physicalPath, string staticFileFilters, string directoryBrowse)
        {
            _port = port;
            _virtualPath = virtualPath;
            _physicalPath = physicalPath.EndsWith("\\", StringComparison.Ordinal) ? physicalPath : physicalPath + "\\";
            _staticFileFilters = staticFileFilters;
            _directoryBrowse = directoryBrowse;
        }
        ...
        public string StaticFileFilters
        {
            get
            {
                return _staticFileFilters;
            }
        }
        public string DirectoryBrowse
        {
            get
            {
                return _directoryBrowse;
            }
        }
