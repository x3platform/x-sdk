...
namespace X3Platform.HttpServer
{
    ...
    class Request : SimpleWorkerRequest
    {
        ...
        public Request(HttpServer server, Host host, Connection connection)
        ...
        public void Process()
        {
            ...
            // 支持响应静态文件
            string extension = Path.GetExtension(_pathTranslated);

            if (!string.IsNullOrEmpty(extension) && _server.StaticFileFilters.IndexOf(extension) > -1)
            {
                _connection.WriteEntireResponseFromFile(_pathTranslated, false);
                return;
            }
            ...
            if (_server.DirectoryBrowse == "true" && ProcessDirectoryListingRequest())
            {
                return;
            }
